...
class MessageProcessor {
	...
	async checkAndPromptMissingInfo(doc, ctx) {
		...
		const isBlockedOrHold =
			doc.status === "BLOCKED" || doc.status === "HOLD";
		const isReasonMissing =
			!doc.blocked_reason || doc.blocked_reason.trim() === "";
	...
	async process(raw, options = {}) {
		...
		try {
			...
			if (
				...
				const statusMatch = lowerReply.match(
					/\bstatus\s*-\s*([a-z]+)\b/i,
				);
				...
				if (statusMatch || priorityMatch) {
					...
					if (statusMatch) {
						...
						if (
							/^(resolve|resolved|done|completed|finished)$/.test(
								s,
							)
						)
					...
					if (Object.keys(updates).length > 0) {
						...
						if (
							...
							(!existing_task.blocked_reason ||
								existing_task.blocked_reason.trim() === "")
						) {
			...
			if (text.trim().toLowerCase().startsWith("delay")) {
				...
				if (targetToTag && slack_client) {
					...
					if (minMatch) {
						...
						const parsedDateStr = extractDueDate(
							timeInput,
							new Date(),
						);
						if (parsedDateStr) {
			...
			if (parsed.task?.title) {
				parsed.task.title = parsed.task.title
					...
					.replace(
						/Task\/Issue requirement based on attached file.*?/gi,
						"",
					)
					.trim();
				if (parsed.task.title.length < 3)
					parsed.task.title = "Task from attached file";
			}
			if (parsed.issue?.title) {
				parsed.issue.title = parsed.issue.title
					.replace(/\[Attachment.*?\]/gi, "")
					.replace(
						/Task\/Issue requirement based on attached file.*?/gi,
						"",
					...
					.trim();
				if (parsed.issue.title.length < 3)
					parsed.issue.title = "Issue from attached file";
			}
