...
// New function: Get dashboard filtered by date (defaults to today)
async function getDashboardForDate(dateStr = null, channel = null) {
  const now = new Date();
  
  // Default to today if no date provided
  const requestedDateStr = dateStr || await getTodayStr();
  ...
  if (channel) {
    ...
    activityFilter.channel = channel;
  }
  ...
  const [
    ...
    Task.find(taskFilter).sort({ updated_time: -1 }).lean(),
    Issue.find(issueFilter).sort({ updated_time: -1 }).lean(),
    Discussion.find(discussionFilter).sort({ timestamp: -1 }).lean(),
    Activity.find(activityFilter).sort({ created_at: -1 }).lean(),
    Notification.find({
      status: { $in: ['PENDING', 'SENT'] },
      type: {
        $in: [
    ...
    Issue.aggregate([{ $match: issueFilter }, { $group: { _id: '$status', count: { $sum: 1 } } }]),
  ]);
...
module.exports = { getDashboard, getDashboardForDate };