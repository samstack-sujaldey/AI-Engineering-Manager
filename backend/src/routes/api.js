...
function createApiRouter({ messageProcessor }) {
	...
	router.get("/discussions/daily-summary", async (req, res) => {
		try {
			...
			res.status(500).json({
				error: "Failed to fetch daily summary cache",
			});
		}
	});
...
`;
			...
			if (!parsedData || !parsedData.member_updates) {
				...
				for (const line of lines) {
					if (
						/^(hi everyone|date|duration|present members|team-wise task updates)/i.test(
							line,
						)
					)
						continue;
					...
					if (!isBullet && line.length < 30 && !line.includes(".")) {
						...
						if (!memberUpdatesMap[currentMember]) {
							memberUpdatesMap[currentMember] = {
								tasks: [],
								issues: [],
								discussions: [],
							};
				...
				parsedData = {
					...
					member_updates: Object.entries(memberUpdatesMap).map(
						([member_name, data]) => ({
							member_name,
							...data,
						}),
			...
			res.status(500).json({
				error: "Failed to parse stand-up MOM message",
			});
		}
	});
