...
const DEPENDENCY_PATTERNS = [
    ...
    { type: "Waiting for Approval", re: /waiting\s+(for|on)\s+(a\s+)?approval/i },
    { type: "Waiting for Merge", re: /waiting\s+(for|on)\s+(a\s+)?merge/i },
    { type: "Waiting for Deployment", re: /waiting\s+(for|on)\s+(a\s+)?deploy/i },
    { type: "Waiting for API", re: /waiting\s+(for|on)\s+(the\s+)?api/i },
    { type: "Waiting for Access", re: /waiting\s+(for|on)\s+access/i },
    { type: "Waiting for Infrastructure", re: /waiting\s+(for|on)\s+(infra|infrastructure)/i },
...
function extractMentionedUsers(text, userDirectory = {}) {
    ...
    Object.values(userDirectory).forEach((u) => {
        ...
        const isMatched = cleanWords.some(
            (w) => w.length > 2 && (w === handle || w === displayName || w === firstName)
        );
...
function extractBlockedReason(text) {
    ...
    const naturalMatch =
        text.match(/blocked\s+(?:because|due\s+to|by|on)\s+(.+?)(?:\.|\n|$)/i) ||
        text.match(/waiting\s+(?:for|on)\s+(.+?)(?:\.|\n|$)/i) ||
        text.match(/cannot\s+continue\s+(?:because|due\s+to)\s+(.+?)(?:\.|\n|$)/i);
...
function extractEntities(text) {
    ...
    const branch = text.match(/\b(?:branch|on)\s+[`'"]?([A-Za-z0-9._/-]+)[`'"]?/i);
    const repo = text.match(/\b(?:repo|repository)\s+[`'"]?([A-Za-z0-9._/-]+)[`'"]?/i);
...
function classifyMessage(text) {
    ...
    if (
        ...
        return { classification: "TASK", confidence: 0.99, needs_human_review: false };
    }
    if (
        ...
        return { classification: "ISSUE", confidence: 0.99, needs_human_review: false };
    }
...
function generateTitle(text, classification) {
    ...
    cleaned = cleaned
        .replace(/\s*-\s*(block(ed)?|done|todo|processing|in progress)\s*$/i, "")
        .trim();
...
function detectWatchersAndReviewers(text, mentionedUsers, assignedTo) {
    ...
    if (/cc\s*:?/i.test(text) || /\bfyi\b/i.test(text) || /\bwatch(ing)?\b/i.test(text)) {
    ...
    if (classification === "ISSUE" && mentionedUsers.length > 0) {
        ...
        if (dependent?.id && dependent.id !== owner?.id) {
            notifications.push({
                ...
                target_user_name: dependent.name || dependent.display_name || "",
...
async function parseMessage(input) {
    ...
    const { classification, confidence, needs_human_review } = classifyMessage(text);
    ...
    if (classification === "TASK") {
        ...
        return buildResponse({
            ...
            message_ts,
            task: {
                id: existing_task ? existing_task.task_id || existing_task.id : "",
    ...
    if (classification === "ISSUE") {
        ...
        return buildResponse({
            ...
            message_ts,
            issue: {
                id: existing_issue ? existing_issue.issue_id || existing_issue.id : "",
    ...
    return buildResponse({
        classification: "GENERAL_DISCUSSION",
        confidence,
        action: linkingTask || linkingIssue ? "LINK_DISCUSSION" : "STORE_DISCUSSION",
        sender: senderUser,
        owner: assignment.needs_assignment ? { id: "", name: "Unassigned" } : null,
        assigned_to: null,
        assigned_by: null,
        reporter: senderUser,
        mentioned_users: mentionedUsers,
        ...
        discussion: {
            ...
            issue_id: linkingIssue ? linkingIssue.issue_id || linkingIssue.id : null,
